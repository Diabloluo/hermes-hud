...
def compute_summary(home: Path, time_range: str = "7d",
    ...
    cost_complete = source_ok and rows_with_cost == usage_rows and usage_rows > 0
    ratio = (rows_with_cost / usage_rows) if usage_rows else (1.0 if source_ok else None)
    if not source_ok:
        cost_total = input_tok = output_tok = total_tokens = None
        rows_with_cost = usage_rows = None
        ratio = None
    elif usage_rows == 0:
...
def compute_timeseries(home: Path, time_range: str = "7d",
    ...
    return {"schema_version": 1, "range": time_range, "points": series,
            "source_status": "healthy" if source_ok else "unavailable"}
...
def compute_models(home: Path, time_range: str = "7d",
    ...
    for r in rows:
        ...
        m = models.setdefault(model, {"model": model, "input_tokens": 0,
                                      ...
                                      "cost_coverage": {"rows": 0, "rows_with_cost": 0}})
        m["input_tokens"] += (r[2] or 0)
        m["output_tokens"] += (r[3] or 0)
        m["total_tokens"] += (r[2] or 0) + (r[3] or 0)
        if r[7] is not None:
            m["estimated_cost_usd"] += float(r[7])
            m["cost_coverage"]["rows_with_cost"] += 1
        m["cost_coverage"]["rows"] += 1
        m["sessions"].add(r[0])
    out = []
    for m in models.values():
        m["sessions"] = len(m["sessions"])
        cc = m["cost_coverage"]
        m["cost_coverage_ratio"] = (cc["rows_with_cost"] / cc["rows"]) if cc["rows"] else 1.0
        out.append(m)
    out.sort(key=lambda x: x["estimated_cost_usd"], reverse=True)
    return {"schema_version": 1, "range": time_range, "models": out,
            "source_status": "healthy" if source_ok else "unavailable"}
...
def compute_top_sessions(home: Path, time_range: str = "7d", limit: int = 20,
    ...
    for r in rows:
        ...
        if s["first_seen"] is None or r[8] < s["first_seen"]:
            s["first_seen"] = r[8]
        if s["last_seen"] is None or r[9] > s["last_seen"]:
            s["last_seen"] = r[9]
    ...
    for s in sess.values():
        ...
        out.append(s)
    out.sort(key=lambda x: x["estimated_cost_usd"], reverse=True)
    return {"schema_version": 1, "range": time_range,
...
def compute_budget(home: Path, daily_budget_usd: float, now: Optional[int] = None) -> dict:
    """Budget view：只读复用 daily_budget_usd；coverage 不完整 → ratio 不精确。"""
    summary = compute_summary(home, "today", now=now)
    today_cost = summary["estimated_cost_usd"]
    if today_cost is None:
        return {"budget_usd": daily_budget_usd, "today_estimated_cost_usd": None,
                "usage_ratio": None, "remaining_usd": None,
                "budget_status": "unavailable", "budget_configured": daily_budget_usd > 0}
    if daily_budget_usd <= 0:
        return {"budget_usd": daily_budget_usd, "today_estimated_cost_usd": today_cost,
                "usage_ratio": None, "remaining_usd": None,
                "budget_status": "not_configured", "budget_configured": False}
    if not summary["cost_complete"]:
        return {"budget_usd": daily_budget_usd, "today_estimated_cost_usd": today_cost,
                "usage_ratio": None, "remaining_usd": None,
                "budget_status": "partial", "budget_configured": True}
    ratio = today_cost / daily_budget_usd
    return {"budget_usd": daily_budget_usd, "today_estimated_cost_usd": today_cost,
            "usage_ratio": ratio, "remaining_usd": daily_budget_usd - today_cost,
            "budget_status": "ok", "budget_configured": True}
