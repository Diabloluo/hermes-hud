...
HOME = Path.home()
REGISTRY_PATH = Path(os.environ.get("HERMES_SKILL_REGISTRY",
                                    HOME / ".hermes" / "skill-registry" / "registry.json"))
SKILLS_ROOT = Path(os.environ.get("HERMES_SKILLS_ROOT", HOME / ".hermes" / "skills"))
LEDGER = Path(os.environ.get("HERMES_JOB_LEDGER",
                             HOME / ".hermes" / "skills" / "job-ledger" / "scripts" / "ledger.py"))
REVIEWER_VERSION = "v1"
STALE_AFTER_HOURS = 24
FUTURE_TOLERANCE_HOURS = 1

# 退出码契约：APPROVE=0 · BLOCK=1 · APPROVE WITH WARNINGS=2 · internal/usage=9
EXIT_APPROVE = 0
EXIT_BLOCK = 1
EXIT_WARN = 2
EXIT_USAGE = 9
...
def load_registry() -> tuple[dict | None, str | None]:
    """返回 (registry, error)。缺失/损坏/过期/未来时间戳 → (None, reason)。

    任何 freshness 问题都不允许静默通过（工单第 3 点：missing/invalid/old/future 全 BLOCK）。
    """
    ...
    if not gen:
        return None, "registry 缺少 generated_at（无法判断新鲜度）"
    try:
        ...
        return None, f"registry generated_at 无效: {gen!r}"
    if age_h < -FUTURE_TOLERANCE_HOURS:
        return None, f"registry generated_at 指向未来（{gen}，超前 {round(-age_h, 1)}h）——时钟/数据异常"
    if age_h > STALE_AFTER_HOURS:
        return None, f"registry 过期（{round(age_h, 1)}h > {STALE_AFTER_HOURS}h）"
    data["_stale"] = False
    return data, None
...
_SIDE_EFFECT_PATTERNS = {
    ...
    "destructive_ops": [r"rm\s+-rf", r"pkill", r"killall", r"delete[^a-z]",
                        r"os\.remove", r"unlink\("],
}


def review_side_effects(skill_dir: Path) -> dict:
    """Side-effect 声明（全文件推断）+ 控制边界（只允许 SKILL.md 判断）。

    契约：禁止从 .py/.sh 中出现的 warning/danger/manual 等词推断 controlled；
    边界词表只保留明确语义（confirmation / human approval / dry-run /
    fail-closed / explicit read-only / destructive prohibition）。
    """
    text_parts = []
    ...
    # 高风险 side effect 的控制边界：**只从 SKILL.md 判断**
    md_text = "\n".join(
    ...
    # 明确语义词表（不含 warning/danger/警告 等模糊词）
    boundary_terms = ["确认", "confirm", "人工", "manual", "approve", "批准", "授权",
                      "只读", "read-only", "不自动", "fail-closed", "dry-run",
                      "禁止", "prohibit", "绝不", "never"]
    has_boundary = any(t in md_text.lower() for t in boundary_terms)
    verdict = _verdict("PASS", f"边界描述(SKILL.md): {'有' if has_boundary else '无'}")
    if high_se and not has_boundary:
        verdict = _verdict("FAIL", f"高风险副作用无边界描述（仅 SKILL.md 判定）: {', '.join(high_se)}")
    elif high_se:
        verdict = _verdict("WARN", f"高风险副作用（SKILL.md 有边界描述）: {', '.join(high_se)}")
...
def review(key: str) -> int:
    """执行审核，返回退出码（APPROVE=0 / BLOCK=1 / WARN=2 / usage=9）。"""
    ...
    if key not in recs:
        print(f"🧩 Skill Review\n\nERROR: registry 中无 {key}（先 skill_registry scan）")
        return EXIT_USAGE
    ...
    if skill_dir is None:
        alt = SKILLS_ROOT / key
        if alt.is_dir():
            skill_dir = alt  # 目录存在但无 SKILL.md → metadata 门 FAIL → BLOCK
        else:
            print(f"🧩 Skill Review\n\nERROR: 磁盘上找不到 skill 目录 {key}")
            return EXIT_USAGE
    ...
    return {"APPROVE": EXIT_APPROVE, "BLOCK": EXIT_BLOCK,
            "APPROVE WITH WARNINGS": EXIT_WARN}[decision]
...
class _Parser(argparse.ArgumentParser):
    """argparse 错误 → _UsageError（CLI 退出码 9）。"""
